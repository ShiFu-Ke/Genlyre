            ...
            for j in i:
                for k in j:
                    if k in MusicTest.keys:
                        if lyre == 1:
                            Thread(daemon=True,
                                   target=lambda: playsound("app/resource/sound/FengWu/" + k + ".mp3")).start()
                        elif lyre == 2:
                            Thread(daemon=True,
                                   target=lambda: playsound("app/resource/sound/LaoJiu/" + k + ".mp3")).start()
                        else:
                            Thread(daemon=True,
                                   target=lambda: playsound("app/resource/sound/JingHua/" + k + ".mp3")).start()
                sleep(time_tmp)
