...
class AutoPlayInterface(GalleryInterface):
    ...
    def __init__(self, parent=None):
        ...
        self.comboBox_head.setMinimumWidth(120)
        ...
        # 下拉框
        self.comboBox_end = ComboBox()  # 琴谱格式
        self.comboBox_end.addItems([self.tr("Windsong Lyre"), self.tr('Vintage Lyre')])
        self.comboBox_end.setCurrentIndex(0)
        self.comboBox_end.setMinimumWidth(120)
        self.comboBox_end.currentIndexChanged.connect(lambda: self.setLyre())
        # 按钮
        ...
        HLayout_end.addWidget(self.comboBox_end, 0, Qt.AlignLeft)
    ...
    def setLyre(self):
        """修改琴类型"""
        self.lyre = self.comboBox_end.currentIndex() + 1
    ...
    def start(self):
        ...
        if tmp == -1:
            # self.showMessageDialog("No element selected")
            return
        self.playMusic.start(self.songList[tmp][0], self.songList[tmp][1], self.lyre)
